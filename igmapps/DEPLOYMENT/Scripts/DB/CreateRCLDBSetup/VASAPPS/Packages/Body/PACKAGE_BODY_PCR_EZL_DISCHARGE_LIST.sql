CREATE OR REPLACE PACKAGE BODY VASAPPS."PCR_EZL_DISCHARGE_LIST" 
as
/*-----------------------------------------------------------------------------------------------------------
...
begin
    --dbms_output.put_line('RCLAPPS.PCR_EZL_DISCHARGE_LIST.PRR_UPD_BOOKED_DISCHARGE: Start');
    ...
    if(table_name=table1)then
        sql_where := ' where dn_equipment_NO='''||p_container_no||'''/*container no*/
                        ...
                        and dn_pol='''||p_pol_pod||'''/*POL*/
                        and dn_pol_terminal='''||p_terminal||'''/*POL terminal*/
                        and rownum=1
                    ';
    ...
    if(cnt_row>0)then
       ...
       if(table_name=table1)then
           sql_update := 'update '||table_name||'
            ...
            sql_update := 'update '||table_name||'
                            set
                            POL_Terminal=upper(nvl('''|| p_terminal ||''',POL_Terminal))/*terminal*/
                         ';
       elsif table_name=table2 and cnt_row>1 then
