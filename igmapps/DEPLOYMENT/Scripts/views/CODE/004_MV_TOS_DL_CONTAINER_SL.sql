...
System  : EZLL
Module  : BAYPLAN
Name    : V_TOS_DL_CONTAINER_SL.sql
Purpose : A join on TOS_DISCHLIST_DTL + ITP063 showing eta, etd from ITP063.
--------------------------------------------------------------------------------
History : None
--------------------------------------------------------------------------------
Author		Date		    What
...
WACCHO1		04/10/2011	v0.31
Dhruv           09/11/2011      v0.32
--Change Log--------------------------------------------------------------------
...
02    12/07/11     Dhruv             Normal view changed to Materialized view.
				     And Renamed to MV_TOS_DL_CONTAINER_SL.
03    04/10/11	   WACCHO1           Remove Dir Checking, some cntr missing 
				     due to direction <> vvpdir.
04    09/11/11     Dhruv             WEIGHT column is added as CONTAINER_GROSS_WEIGHT to fetch weight value.
--------------------------------------------------------------------------------
**/

--DROP VIEW MV_TOS_DL_CONTAINER_SL;  --##02
DROP MATERIALIZED  VIEW MV_TOS_DL_CONTAINER_SL; --##03

CREATE MATERIALIZED VIEW "MV_TOS_DL_CONTAINER_SL" --##02
("DL_SERVICE",
"DL_VESSEL",
"DL_VOYAGE",
"DL_DIRECTION",
...
AS
SELECT /*+ PARALLEL(IT,4) PARALLEL(DL,4)*/ --##03
    DL.POD_SERVICE		DL_SERVICE
   ...
   ,DL.WEIGHT			DL_CONTAINER_GROSS_WEIGHT --##04
FROM MV_ITP063	       IT
...
WHERE VVVERS = 99
  ...
  --AND IT.VVPDIR = DL.POD_DIR  --##01,##03
